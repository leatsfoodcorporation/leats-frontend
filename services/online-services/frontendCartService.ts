 ...
 * inventoryProductId + variantIndex + selectedCuttingStyle uniquely identify a cart item
 */
 ...
 * inventoryProductId + variantIndex + selectedCuttingStyle uniquely identify a cart item
 * This allows same inventory item with different variants (500g vs 1kg) to be separate cart items
 */
export const removeFromCart = async (
  ...
  const response = await axiosInstance.delete(`/api/online/cart/${inventoryProductId}`, {
    params: { userId, variantIndex, selectedCuttingStyle }
  });
  return response.data;
};
