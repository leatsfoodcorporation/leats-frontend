...
export default function FrequentlyBoughtTogether({
  ...
  const handleAddAllToCart = async () => {
    try {
      ...
      // Add main product only if it has valid inventoryProductId
      if (mainProduct.inventoryProductId) {
        itemsToAdd.push({
      ...
      // Handle errors properly - check if it's an Error object
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="text-center max-w-[80px] sm:max-w-[96px]">
                        <p className="text-[10px] sm:text-xs text-gray-700 line-clamp-2 mb-1">
                          ...
                          {addon.variant.variantUom && addon.variant.variantUomValue && (
                            <span className="text-gray-500 ml-0.5">
                              ({addon.variant.variantUomValue}{addon.variant.variantUom})
                            </span>
                          )}
                        </p>
                        <p className="text-xs sm:text-sm font-semibold text-gray-900">
