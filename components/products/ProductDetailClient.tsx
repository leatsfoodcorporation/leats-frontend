...
export default function ProductDetailClient({
  ...
  useEffect(() => {
    // Auto-select first cutting style if available
  ...
  // ✅ FIX: For combo products, use product.id as inventoryProductId
  const inventoryProductId = product?.type === 'combo' 
    ? product.id 
    : (currentVariant?.inventoryProductId || "");
  const quantity = product
    ? getItemQuantity(product.id, inventoryProductId, selectedVariant, selectedCuttingStyle)
    : 0;
  ...
  const handleAddMultipleToCart = async (items: Array<{ inventoryProductId: string; quantity: number }>) => {
    // For combo products, inventoryProductId is product.id
    // For regular products, it's currentVariant?.inventoryProductId
    const mainProductInventoryId = inventoryProductId;
    ...
    for (const addonItem of addonItems) {
      ...
      if (addon && addon.product && addon.variant) {
        ...
        const addonProductData: Product = {
          id: addon.product.id,
          ...
          stockStatus: addon.product.stockStatus,
  ...
  const handleIncrement = () => {
    ...
    // If item is not in cart yet (quantity === 0), add it first
    if (quantity === 0) {
      addToCart(product, selectedVariant, selectedCuttingStyle || undefined);
    } else {
      updateQuantity(product.id, inventoryProductId, selectedVariant, quantity + 1, selectedCuttingStyle || undefined);
    }
  };
  ...
  const getShareText = () => {
    ...
    return `Check out ${currentVariant.displayName || currentVariant.variantName || product.shortDescription} - ${currencySymbol}${price}`;
  };
        ...
        if (navigator.share) {
          try {
            await navigator.share({
              title: currentVariant?.displayName || currentVariant?.variantName || product?.shortDescription || 'Product',
              text: text,
              url: url,
            });
          } catch (err) {
  ...
  // ✅ FIX: Calculate discount based on type
  ...
  const discount = isFlatDiscount && discountValue > 0
    ? discountValue // Use flat discount value
    : discountPercentage; // Use calculated percentage
  ...
  // ✅ FIX: For combo products, use thumbnail as fallback if variant images are empty
  const variantImages = currentVariant?.variantImages || [];
  const productImages =
    variantImages.length > 0 && variantImages[0] && variantImages[0].trim() !== ''
      ? variantImages 
      : product.type === 'combo' && product.thumbnail && product.thumbnail.trim() !== ''
        ? [product.thumbnail] 
        : [];
          ...
          <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm overflow-x-auto">
            ...
            <span className="text-gray-800 font-medium truncate">
              {product.type === 'combo' 
                ...
                : (currentVariant?.displayName || currentVariant?.variantName || product.shortDescription)}
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
            ...
            <div>
              ...
              <div className="flex items-start justify-between gap-3 mb-2 sm:mb-3">
                <h1 className="text-lg sm:text-2xl font-bold text-gray-800 flex-1 flex items-center gap-2">
              ...
              {/* Cutting Styles - Show for all products including combo */}
              ...
              {frequentlyBoughtTogether.length > 0 && currentVariant && (
                <FrequentlyBoughtTogether
                  mainProduct={{
                    id: product.id,
                    name: currentVariant.displayName || currentVariant.variantName || product.shortDescription,
                    price: price,
                    mrp: mrp,
                    image: currentImage,
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <tbody className="divide-y divide-gray-100">
                  <tr>
                    <td className="py-2.5 sm:py-3 pr-4 text-gray-500 w-1/3 sm:w-1/4">Cash on Delivery</td>
                    <td className="py-2.5 sm:py-3 text-gray-800">
