...
export const SignUp = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.phoneNumber.trim()) {
      ...
      // More flexible validation - just check if it has reasonable length and contains digits
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        const errorMessage = errorResponse?.response?.data?.error || "Network error. Please try again.";
        ...
        if (errorDetails && Array.isArray(errorDetails)) {
          ...
          errorDetails.forEach(detail => {
            if (detail.includes('email')) {
              ...
              setErrors(prev => ({ ...prev, phoneNumber: detail }));
            } else if (detail.includes('password')) {
              setErrors(prev => ({ ...prev, password: detail }));
            } else if (detail.includes('name')) {
              setErrors(prev => ({ ...prev, fullName: detail }));
            }
          });
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      try {
        ...
        // Continue with Google OAuth even if FCM fails
      }
