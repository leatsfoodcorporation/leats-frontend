...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const activeVariants = product.variants.filter((v: typeof product.variants[0]) => v.variantStatus === "active");
  
  // Find the default variant index from active variants, fallback to 0
  const defaultVariantIndex = activeVariants.findIndex((v: typeof product.variants[0]) => v.isDefault);
  ...
  // ✅ FIX: Get correct inventory product ID for both regular and combo products
  const inventoryProductId = product.type === 'combo' 
    ? product.id 
    : (currentVariant?.inventoryProductId || '');
  
  // ✅ FIX: Get actual variant index from original variants array (not filtered activeVariants)
  const actualVariantIndex = product.variants.findIndex(
    (v: typeof product.variants[0]) => v.inventoryProductId === currentVariant?.inventoryProductId
  );
  
  const quantity = getItemQuantity(product.id, inventoryProductId, actualVariantIndex >= 0 ? actualVariantIndex : selectedVariant);

  // ✅ FIX: Check stock availability for both regular and combo products
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    // ✅ FIX: Validate stock before adding to cart
    if (isOutOfStock) {
      return; // Don't add if out of stock
    }
    
    await addToCart(product, actualVariantIndex >= 0 ? actualVariantIndex : selectedVariant);
  };

  const handleIncrement = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    updateQuantity(product.id, inventoryProductId, actualVariantIndex >= 0 ? actualVariantIndex : selectedVariant, quantity + 1);
  };
    ...
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow group relative h-full flex flex-col">
      ...
      <Link href={`/products/${product.id}`} className="block">
        <div className="relative aspect-square bg-gray-50 p-2 sm:p-4">
          {/* ✅ FIX: Show out of stock overlay */}
          {isOutOfStock && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-10 rounded-t-lg">
              <span className="bg-red-600 text-white text-xs sm:text-sm font-bold px-3 py-1.5 rounded">
                OUT OF STOCK
              </span>
            </div>
          )}
      ...
      <div className="p-2 sm:p-3 flex flex-col flex-grow">
        ...
        <div className="relative mb-2 sm:mb-3 h-8 sm:h-10">
          ...
          {showVariants && (
            <div className="absolute top-full left-0 mt-1 w-[280px] sm:w-[320px] bg-white border border-gray-200 rounded-lg shadow-lg z-20">
              {activeVariants.slice(0, 5).map((variant: typeof product.variants[0], index: number) => (
                <button
                  key={index}
                  onClick={(e) => {
                    e.preventDefault();
        ...
        <div className="flex items-center gap-2 mt-auto">
          {isOutOfStock ? (
            <button
              disabled
              className="flex-1 py-1.5 sm:py-2 px-2 sm:px-4 bg-gray-300 text-gray-500 rounded font-medium cursor-not-allowed text-xs sm:text-sm"
            >
              Out of Stock
            </button>
          ) : quantity === 0 ? (
