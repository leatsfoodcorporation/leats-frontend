...
export default function DynamicProductCard({
  ...
  // ✅ Get the actual variant index from the full product.variants array
  ...
  const quantity = getItemQuantity(product.id, inventoryProductId, actualVariantIndex, selectedCuttingStyle || undefined);
  const productUrl = generateProductUrl(product);

  // Get image from variant or use placeholder
  // ✅ FIX: For combo products, use thumbnail if variant images are empty
  const productImage =
  ...
  // ✅ FIX: Calculate discount properly based on discount type
  ...
  // Get discount type and value from variant or product level
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    // IMPORTANT: Multiple variants can have the same inventoryProductId (e.g., 500g and 1kg of same item)
    // So we need to match by BOTH inventoryProductId AND variantUomValue to get the correct variant
    const actualVariantIndex = product.variants.findIndex(
      v => v.inventoryProductId === currentVariant?.inventoryProductId &&
           v.variantUomValue === currentVariant?.variantUomValue
    );
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 sm:top-2 sm:right-2 flex flex-col items-end gap-1 z-10">
       
        {discountText && (
          <span className="bg-green-600 text-white text-xs sm:text-xs font-bold px-2 sm:px-2 py-1 rounded">
            {discountText}
          </span>
        )}
      </div>
        ...
        <div className="mb-2 h-5 flex items-center">
          {isLowStock && quantity === 0 && (
            <span className="text-xs sm:text-xs text-orange-600 font-medium">
              Only {currentVariant?.variantUom ? formatSmartUOMDisplay(availableStock, currentVariant.variantUom) : availableStock} left in stock
            </span>
          )}
