...
export function POSInvoiceView({ order, companySettings, isOpen, onClose }: POSInvoiceViewProps) {
  ...
  const handlePrint = async () => {
    ...
    const itemsHtml = order.items.map((item: POSOrderItem) => {
      ...
      return `
      <div class="item-row">
        <div class="item-main">${item.productName}</div>
        <div class="item-cols">
          <span class="col-qty">${qtyOnly}</span>
          <span class="col-price">${formatThermalCurrency(item.unitPrice)}</span>
          <span class="col-gst">${gstPercentage > 0 ? `${formatThermalCurrency(gstAmount)}(${Math.round(gstPercentage)}%)` : '-'}</span>
          <span class="col-total">${formatThermalCurrency(item.total)}</span>
        </div>
        ${item.productSku ? `
          <div class="item-note">
            SKU: ${item.productSku}
        ...
        <head>
          ...
          <style>
            * {
            ...
            .items-header {
              display: grid;
              grid-template-columns: 2.8fr 1fr 1.4fr 1.2fr 1.4fr;
              gap: 4px;
              font-size: 11px;
              font-weight: bold;
              border-bottom: 1px dashed #000;
            ...
            .items-header span { white-space: nowrap; }
            .item-row { margin: 4px 0 6px; }
            .item-main {
              font-size: 11px;
              margin-bottom: 1px;
            ...
            .item-cols span { white-space: nowrap; }
            .col-qty { text-align: center; }
            .col-price, .col-gst, .col-total { text-align: right; }
          ...
          <div class="totals">
            ...
            ${(order.roundingOff ?? 0) !== 0 ? `
              <div class="total-row">
                ...
                <span>${formatThermalCurrency(order.roundingOff)}</span>
              </div>
            ` : ''}
            ...
            ${order.amountReceived ? `
              <div class="total-row">
                ...
                <span>${formatThermalCurrency(order.amountReceived)}</span>
              </div>
            ` : ''}
            ${(order.changeGiven ?? 0) > 0 ? `
              <div class="total-row">
                <span>Change Given:</span>
                <span>${formatThermalCurrency(order.changeGiven)}</span>
              </div>
            ` : ''}
          </div>
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 32px;">
            <thead>
              <tr style="border-bottom: 2px solid #1f2937;">
                ...
                <th style="text-align: center; padding: 12px 8px; font-size: 12px; font-weight: bold; color: #1f2937; text-transform: uppercase; letter-spacing: 1px;">Qty/Unit</th>
              ...
              ${order.items.map((item: POSOrderItem, index) => {
                return `
                  <tr style="border-bottom: 1px solid #e5e7eb; ${index % 2 === 1 ? 'background-color: #f8fafc;' : ''}">
                    ...
                    <td style="padding: 12px 8px; text-align: center; font-weight: 600; color: #1f2937;">${formatOrderItemQuantity(item.quantity, item.variantUom || undefined, item.variantUomValue || undefined)}</td>
            ...
            <div ref={printRef}>
              ...
              <div className="border-t border-b border-dashed border-gray-800 py-2 my-3">
                <div className="grid grid-cols-[2fr_0.5fr_1fr_1.2fr_1fr] gap-1 text-xs font-bold border-b border-gray-800 pb-1 mb-2">
                  ...
                  <span className="text-center">Qty/UOM</span>
                  ...
                  return (
                    <div key={index} className="mb-2">
                      <div className="grid grid-cols-[2fr_0.5fr_1fr_1.2fr_1fr] gap-1 text-xs">
                        ...
                        <span className="text-center">{formatOrderItemQuantity(item.quantity, item.variantUom || undefined, item.variantUomValue || undefined)}</span>
