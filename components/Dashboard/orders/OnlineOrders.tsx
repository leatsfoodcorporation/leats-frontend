...
interface OnlineOrder {
  ...
  deliveryAcceptedAt?: string; // When partner accepted
  deliveryPickedAt?: string; // When partner picked up
  estimatedDeliveryTime?: string;
  deliveryNotes?: string;
  // Scheduling fields
  orderType?: 'LIVE' | 'PRE_ORDER';
...
export function OnlineOrders() {
  ...
  const [assignmentOrder, setAssignmentOrder] = useState<OnlineOrder | null>(null);
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 32px;">
            ...
            <tbody>
              ${order.items.map((item, index) => {
                ...
                // Format quantity with UOM outside the template string
                const qtyDisplay = formatOrderItemQuantity(
                  item.quantity, 
                  item.variantUom, 
                  item.variantUomValue
                );
                ...
                return `
                  <tr style="border-bottom: 1px solid #e5e7eb; ${index % 2 === 1 ? 'background-color: #f8fafc;' : ''}">
                    ...
                    <td style="padding: 12px 8px; text-align: center; font-weight: 600; color: #1f2937;">${qtyDisplay}</td>
  ...
  const handleStatusChange = async (orderId: string, newStatus: string, currentStatus: string) => {
    ...
    // If changing to delivered, open payment modal
    if (newStatus === 'delivered') {
      const order = orders.find(o => o.id === orderId);
      if (!order) return;
      
      setPendingStatusUpdate({ orderId, newStatus, currentStatus, order });
      setPaymentStatus('completed'); // Always set to completed when delivered
  ...
  const handlePaymentUpdate = async () => {
    ...
    try {
      setIsUpdatingStatus(pendingStatusUpdate.orderId);
      const response = await axiosInstance.patch(
        `/api/online/admin/orders/${pendingStatusUpdate.orderId}/status`,
        { 
      ...
      if (response.data.success) {
        toast.success(
          `Order marked as delivered and payment updated`,
          {
            description: `Payment: ${paymentStatus} via ${paymentMethod}`
          }
        );
        loadOrders();
      ...
      const axiosError = error as { response?: { data?: { error?: string; message?: string } } };
      const errorMessage = axiosError.response?.data?.error || "Failed to update order";
  ...
  const getPaymentStatusBadge = (status: string, orderStatus?: string) => {
    const displayStatus = orderStatus === "cancelled" && status !== "completed" ? "cancelled" : status;
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium ${
          colors[displayStatus] || colors.pending
        }`}
      >
        {displayStatus}
      </span>
  ...
  // Calculate elapsed time since partner assignment
  const getElapsedTime = (order: OnlineOrder) => {
    if (!order.deliveryPartnerId || !order.deliveryAssignAt) {
      return "-";
    }

    const assignedTime = new Date(order.deliveryAssignAt).getTime();
    const currentTime = new Date().getTime();
    
    // If order is delivered, calculate time from assignment to delivery
    let endTime = currentTime;
    if (order.orderStatus === 'delivered' && order.deliveredAt) {
      endTime = new Date(order.deliveredAt).getTime();
    }
    
    const diffMs = endTime - assignedTime;
  ...
  return (
    <div className="space-y-6">
                ...
                <TableRow key={order.id}>
                  ...
                  <TableCell>
                    <div className="text-xs font-medium">
                      {order.scheduledSlot === 'LIVE' ? (
                        <span className="text-green-600 font-bold italic underline">Immediate</span>
                      ) : (
                        order.scheduledSlot || '-'
                      )}
                    </div>
                  ...
                  <TableCell className="text-sm">
                    <div className="flex items-center gap-1">
                      ...
                      <span className={order.deliveryPartnerId ? "font-medium" : "text-gray-400"}>
                    ...
                    <div className="flex gap-1">
                      ...
                      {order.orderStatus !== 'pending' && order.orderStatus !== 'cancelled' && (
                        <Button
                          ...
                          title={order.deliveryPartnerId ? `Assigned to ${order.deliveryPartner?.name}` : "Assign delivery partner"}
                          className={order.deliveryPartnerId ? "bg-green-600 hover:bg-green-700" : ""}
                        >
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="space-y-3 bg-gray-50 p-3 rounded">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{selectedOrder.deliveryPartner?.phone}</p>
                      <p className="text-xs text-gray-500">
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedOrder.deliveryAssignAt && (
                        <div className="flex items-center gap-2 text-sm">
                          <IconTruckDelivery size={16} className="text-blue-600" />
                          <span className="text-gray-600">Assigned:</span>
                          <span className="font-medium">{formatDate(selectedOrder.deliveryAssignAt)}</span>
                        </div>
                      )}
                      {selectedOrder.deliveryAcceptedAt && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium">{formatDate(selectedOrder.deliveryAcceptedAt)}</span>
                        </div>
                      )}
                      {selectedOrder.deliveryPickedAt && (
                        <div className="flex items-center gap-2 text-sm">
                          <IconPackage size={16} className="text-orange-600" />
                          <span className="text-gray-600">Picked Up:</span>
                          <span className="font-medium">{formatDate(selectedOrder.deliveryPickedAt)}</span>
                        </div>
                      )}
                      {selectedOrder.deliveredAt && (
                        <div className="flex items-center gap-2 text-sm">
                          <IconTruck size={16} className="text-purple-600" />
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <div className="text-sm text-muted-foreground">
                  {pendingStatusUpdate.order.orderNumber}
                </div>
              </div>
            ...
            <div className="flex justify-end gap-2">
              <Button
                ...
                disabled={isUpdatingStatus === pendingStatusUpdate.orderId}
              >
                Cancel
              </Button>
              <Button
                ...
                disabled={isUpdatingStatus === pendingStatusUpdate.orderId}
              >
                {isUpdatingStatus === pendingStatusUpdate.orderId ? (
                  <>
      ...
      <DeliveryAssignmentModal
        ...
        orderNumber={assignmentOrder?.orderNumber || ""}
        currentPartnerId={assignmentOrder?.deliveryPartner?.partnerId}
        currentPartnerName={assignmentOrder?.deliveryPartner?.name}
        orderStatus={assignmentOrder?.orderStatus || ""}
        orderType={assignmentOrder?.orderType}
        scheduledDate={assignmentOrder?.scheduledDate}
        scheduledSlot={assignmentOrder?.scheduledSlot}
