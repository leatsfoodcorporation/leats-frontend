...
import { Truck, Clock, Users, Package, TrendingUp, PieChart as PieChartIcon, XCircle } from "lucide-react";
...
export function DeliveryPerformance({ data, dateRange }: DeliveryPerformanceProps) {
  ...
  const deliveryData: DeliveryDataItem[] = [
    { name: "Successful", value: data.successfulDeliveries },
  ...
  const statusData: StatusDataItem[] = [
    ...
    { name: "Picked Up", value: data.pickedUp, color: "#8b5cf6" },
    { name: "In Transit", value: data.inTransit, color: "#06b6d4" },
    ...
    <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
      ...
      <CardContent>
        <div className="space-y-4">
          {/* Chart */}
          {!hasData ? (
            <div className="flex items-center justify-center h-[200px] bg-slate-50 dark:bg-slate-800 rounded-lg">
              <div className="text-center">
                <Package className="w-12 h-12 mx-auto mb-2 text-slate-300 dark:text-slate-600" />
                <p className="text-sm text-slate-500 dark:text-slate-400">No delivery data available</p>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Data will appear when deliveries are made</p>
              </div>
            </div>
          ) : viewType === 'donut' ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">Active Partners</span>
              </div>
            ...
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">Available</span>
              </div>
              <div className="text-xl font-bold text-slate-900 dark:text-slate-100">{data.availableRiders}</div>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                <Package className="w-3 h-3 text-green-500 dark:text-green-400" />
                <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">Successful</span>
              </div>
              <div className="text-xl font-bold text-slate-900 dark:text-slate-100">{data.successfulDeliveries}</div>
            </div>
