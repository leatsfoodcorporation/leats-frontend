...
export function VariantsTab({
  ...
  const fetchInventoryProducts = async () => {
    try {
      ...
      // 🆕 Pass productId to exclude current product from "used" check
      const response = await onlineProductService.getProducts(
        undefined, // No search parameter - SearchableSelect handles filtering
        productId // Pass product ID for edit mode
      );
      if (response.success) {
    ...
    const updatedVariants = formData.variants.map((v) => {
      if (v.id === variantId) {
        ...
        if (field === "variantSellingPrice") {
          const currentSellingPrice = parseFloat(value as any) || 0;
          ...
          const currentMRP = parseFloat(value as any) || 0;
          const prices = calculatePricesSmartly(
            variantId,
          ...
          const currentDiscount = parseFloat(value as any) || 0;
          const prices = calculatePricesSmartly(
            variantId,
            v.variantSellingPrice || 0,
            v.variantMRP || 0,
  ...
  const handleSelectFromInventory = (productId: string, variantId: string) => {
    ...
    // 🔒 DUPLICATE CHECK 1: Show warning if used in another online product
    if (product.isUsedInOnlineProduct) {
      toast.warning(
        `Note: This inventory item "${product.itemName}" is already used in another online product.`,
        { duration: 4000 }
      );
    }
    ...
    const baseVariantData = {
      ...
      // 🆕 Multi-UOM fields from inventory
      variantUom: product.baseUom || product.uom,
      variantUomValue: 1, // Default to 1 (e.g., 1kg)
          ...
          {formData.variants.map((variant, index) => (
            ...
            >
              ...
              <AccordionContent className="px-3 sm:px-6 pb-4 sm:pb-6 pt-4 sm:pt-6 space-y-4 sm:space-y-6">
                  ...
                  <SearchableSelect
                    options={inventoryProducts.map((product) => {
                      const expiryText = product.expiryDate 
                        ...
                        : ' | No Expiry';
                      const usedIndicator = product.isUsedInOnlineProduct ? " (Used)" : "";
                      
                      return {
                        value: product.id,
                        label: product.itemName,
                        description: `${product.itemCode ? `SKU: ${product.itemCode}` : ''}${expiryText}${usedIndicator}`,
                      };
                    })}
                    value=""
                    ...
                    placeholder={
                      isLoadingInventory ? "Loading..." : "Search and select product..."
                    }
                ...
                {/* 🆕 Row for UOM Selection */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 p-3 bg-muted/30 rounded-lg">
                  <div>
                    <Label className="text-sm font-semibold">
                      Variant UOM <span className="text-red-500">*</span>
                    </Label>
                    <Select
                      value={(variant as any).variantUom || ""}
                      onValueChange={(value) => handleUpdateVariant(variant.id, "variantUom" as any, value)}
                      disabled={!variant.inventoryProductId}
                    >
                      ...
                      <SelectContent>
                        {(() => {
                          const product = inventoryProducts.find(p => p.id === variant.inventoryProductId);
                          if (!product) return <SelectItem value="none" disabled>Select inventory item first</SelectItem>;
                          
                          // Get available UOMs from inventory product
                          // 🆕 FIX: Check for array existence AND length to correctly fallback to base UOM
                          const inventoryUoms = (product as any).availableUoms;
                          const availableUoms = (inventoryUoms && Array.isArray(inventoryUoms) && inventoryUoms.length > 0)
                            ? inventoryUoms 
                            : [{ uom: product.baseUom || product.uom, conversionFactor: 1 }];
                          
                          console.log(`🔍 Variant ${variant.id} - Available UOMs:`, availableUoms);
                          
                          return availableUoms.map((u: any) => (
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Select a unit from the inventory item&apos;s available options
                    </p>
                  </div>
                  <div>
                    <Label className="text-sm font-semibold">
                    ...
                    <Input
                      ...
                      step="any"
                      min="0.001"
                      value={(variant as any).variantUomValue || ""}
                      onChange={(e) => handleUpdateVariant(variant.id, "variantUomValue" as any, parseFloat(e.target.value) || 0)}
                      placeholder="e.g., 500 for 500g, 1 for 1kg"
                      disabled={!variant.inventoryProductId}
                      className="mt-2 bg-white"
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      Value for the selected UOM (e.g., 500 for 500g variant)
                    </p>
                  </div>
                </div>
