...
import { getUOMSymbol } from "@/lib/uom-constants";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.expectedDeliveryDate) {
      toast.error("Please select a delivery date", {
        description: "Expected delivery date is required for the purchase order",
      });
      return;
    }
    ...
    if (!formData.items || formData.items.length === 0) {
      toast.error("Please add at least one item", {
        description: "A purchase order must contain at least one item",
      });
      return;
    }
                ...
                {formData.items.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="p-2">
                        {(() => {
                          const selectedInvItem = items.find(i => i.id === item.itemId);
                          const availableUoms = selectedInvItem?.availableUoms || [];
                          ...
                          if (availableUoms.length > 1) {
                            return (
                              ...
                              >
                                ...
                                <SelectContent>
                                  {availableUoms.map((uomObj: any) => (
                                    <SelectItem key={uomObj.uom} value={uomObj.uom}>
                                      {uomObj.uom}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
