...
export const PurchaseSuccessModal: React.FC<PurchaseSuccessModalProps> = ({
  ...
  const formatThermalCurrency = (amount: number) => {
  ...
  const handlePrint = async () => {
    ...
    const itemsHtml = cartItems.map(item => {
      ...
      const quantityDisplay = item.variantUom && item.variantUomValue 
        ? formatOrderItemQuantity(item.quantity, item.variantUom, item.variantUomValue)
        : item.quantity;
      ...
      return `
        <div class="item-row">
          ...
          <div class="item-cols">
            ...
            <span class="col-price">${formatThermalCurrency(unitPriceBeforeGst)}</span>
            <span class="col-gst">${gstPercentage > 0 ? `${formatThermalCurrency(gstAmount)}(${gstPercentage}%)` : '-'}</span>
            <span class="col-total">${formatThermalCurrency(finalAmount)}</span>
          </div>
          ${item.discount && item.discount > 0 ? `
            <div class="item-note">
              Disc: ${item.discountType === "flat" ? formatThermalCurrency(item.discount) : `${item.discount}%`}
            </div>
          ` : ''}
        </div>
      `;
      ...
      <html>
        ...
        <body>
          ...
          <div class="totals">
            <div class="total-row">
              <span>Subtotal:</span>
              <span>${formatThermalCurrency(subtotal)}</span>
            </div>
            ${subtotal !== total ? `
              <div class="total-row" style="color: #16a34a;">
                <span>Discount:</span>
                <span>-${formatThermalCurrency(subtotal - total)}</span>
              </div>
            ` : ''}
            <div class="total-row">
              <span>Total:</span>
              <span>${formatThermalCurrency(total)}</span>
            </div>
            ${roundingOff !== 0 ? `
              <div class="total-row">
                <span>Rounding Off:</span>
                <span style="color: ${roundingOff >= 0 ? '#16a34a' : '#dc2626'};">
                  ${roundingOff >= 0 ? '+' : ''}${formatThermalCurrency(Math.abs(roundingOff))}
                </span>
              </div>
            ` : ''}
            <div class="total-row grand-total">
              <span>FINAL TOTAL:</span>
              <span>${formatThermalCurrency(total + roundingOff)}</span>
            </div>
          </div>
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      cartItems.forEach((item) => {
        ...
        pdf.text(quantityDisplay, 36, yPos, { align: 'center' });
