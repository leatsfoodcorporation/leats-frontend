...
interface Product {
  ...
  display: string;
  itemImage?: string;
  baseUom?: string; // Base UOM from inventory
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const params: Record<string, string> = {
        excludeProcessing: 'true', // 🆕 Exclude processing items
      };
      if (selectedCategory !== "all") {
        params.category = selectedCategory;
      }

      // Fetch inventory items (master data) - excluding processing items
              ...
              currentProducts.map((product) => (
                <TableRow key={product.id}>
                  ...
                  <TableCell>
                    ...
                    >
                      {product.quantity > 0 
                        ? formatSmartUOMDisplay(product.quantity, product.baseUom || 'pcs')
                        : "Out of stock"}
                    </Badge>
                  </TableCell>
                  <TableCell>
