  ...
  useEffect(() => {
    if (!open) {
      ...
      setFormData({
        ...
        uom: initialData.uom || "",
        // 🆕 Load multi-UOM fields
        baseUom: initialData.baseUom || initialData.uom || "",
        selectedUoms: initialData.selectedUoms || (initialData.uom ? [initialData.uom] : []),
        uomLocked: initialData.uomLocked || false,
        purchasePrice: initialData.purchasePrice?.toString() || "",
        gstRateId: initialData.gstRateId || "",
        gstPercentage: initialData.gstPercentage?.toString() || "",
        hsnCode: initialData.hsnCode || "",
        warehouse: initialData.warehouse || "",
        openingStock: initialData.openingStock?.toString() || "",
        lowStockAlertLevel: initialData.lowStockAlertLevel?.toString() || "",
        status: initialData.status || "in_stock",
        expiryDate: initialData.expiryDate,
        description: initialData.description || "",
        itemImage: initialData.itemImage || null,
        itemType: initialData.itemType || "regular",
        requiresProcessing: initialData.requiresProcessing || false,
      });
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="baseUom">
                Base Unit of Measurement (UOM) <span className="text-destructive">*</span>
                {formData.uomLocked && editMode && (
                  <span className="ml-2 text-xs text-amber-600 dark:text-amber-400">🔒 Locked</span>
                )}
              </Label>
              
              {/* 🆕 Category-Specific UOM Selector */}
              {formData.category ? (
                <div className="space-y-3">
                  {/* Consolidated UOM Selection (Base + Additional) */}
                  <div className="space-y-3">
                    <MultipleSelect
                      className="w-full"
                      placeholder="Select units (First one will be Base UOM)..."
                      selectedKeys={new Set(formData.selectedUoms || [])}
                      isDisabled={formData.uomLocked && editMode}
                      onSelectionChange={(keys) => {
                        const valueArr = Array.from(keys) as string[];
                        setFormData(prev => ({
                          ...prev,
                          selectedUoms: valueArr,
                          baseUom: valueArr[0] || "",
                          uom: valueArr[0] || "" // Keep legacy field in sync
                        }));
                      }}
                    >
                      <MultipleSelectContent items={COMMON_UOMS}>
                        {(uom) => (
                          <MultipleSelectItem id={uom.id} textValue={uom.name}>
                            <div className="flex flex-col">
                              <span className="text-sm font-medium">{uom.name}</span>
                    ...
                    {formData.selectedUoms && formData.selectedUoms.length > 0 && (
                      <div className="p-2 bg-blue-50/50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-md">
                        <p className="text-xs font-medium text-blue-700 dark:text-blue-300">
                          🎯 Base UOM: <span className="font-bold underline">{formData.baseUom || formData.uom}</span>
                        </p>
                        <p className="text-[10px] text-muted-foreground">
                          {formData.selectedUoms.length > 1 ? `Additional: ${formData.selectedUoms.slice(1).join(', ')}` : 'No additional units selected'}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {formData.uomLocked && editMode && (
                    <div className="p-3 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-md">
                      <p className="text-sm text-amber-800 dark:text-amber-200">
                        🔒 <strong>UOM Locked:</strong> This item is used in online products. UOM cannot be changed.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="p-3 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-md">
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    ℹ️ Please select a category first to see relevant UOMs
                  </p>
                </div>
              )}
