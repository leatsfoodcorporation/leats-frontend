...
interface AdjustmentItem {
  ...
  itemCode: string;
  category: string;
  uom: string; // Base UOM from product
  selectedUom: string; // Selected UOM for adjustment
  availableUoms: { uom: string; conversionFactor: number }[];
  currentStock: number; // In base UOM
  adjustment: number; // In selected UOM
  reason: string;
  notes: string;
}
...
export default function StockAdjustmentPage() {
    ...
    setItems(items.map((item) => {
      if (item.id === id) {
        if (field === "adjustment") {
          ...
          const uomInfo = item.availableUoms.find((u) => u.uom === item.selectedUom);
          const factor = uomInfo?.conversionFactor || 1;
          const newStock = item.currentStock + (val * factor);
          
          if (newStock < -0.0001) {
            const maxDecrease = -(item.currentStock / factor);
            toast.error(`Adjustment cannot exceed available stock (${formatSmartUOMDisplay(item.currentStock, item.uom)})`);
            return { ...item, adjustment: Number(maxDecrease.toFixed(2)) };
          }
        }
  ...
  const validateForm = (): boolean => {
    ...
    for (const item of items) {
      ...
      const uomInfo = item.availableUoms.find((u) => u.uom === item.selectedUom);
      const factor = uomInfo?.conversionFactor || 1;
      const adjustmentInBase = item.adjustment * factor;
      const newStock = item.currentStock + adjustmentInBase;

      if (newStock < 0) {
        toast.error(`Adjustment for ${item.itemName} would result in negative stock (${formatSmartUOMDisplay(newStock, item.uom)})`);
        return false;
      }
                              ...
                              >
                                ...
                                <div className="flex flex-col flex-1">
                                  ...
                                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                    ...
                                    <span>•</span>
                                    <span>Stock: {formatSmartUOMDisplay(product.quantity, product.uom)}</span>
                                  </div>
                                </div>
                              </CommandItem>
                            );
                    ...
                    items.map((item) => {
                      return (
                        <TableRow key={item.id} className="hover:bg-muted/50">
                          {(() => {
                            const uomInfo = item.availableUoms.find((u: any) => u.uom === item.selectedUom);
                            const factor = uomInfo?.conversionFactor || 1;
                            const adjustmentInBase = item.adjustment * factor;
                            const newStock = item.currentStock + adjustmentInBase;
                              ...
                              <>
                                ...
                                <TableCell className="text-center">
                                  <div className="inline-flex flex-col items-center">
                                    <span className="text-xl font-semibold">{formatSmartUOMDisplay(item.currentStock, item.uom)}</span>
                                  </div>
                                </TableCell>
                          <TableCell>
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              <Input
                                ...
                                className={cn(
                                  ...
                                  newStock < 0 && "border-destructive bg-destructive/10 text-destructive"
                                )}
                                value={item.adjustment}
                                step="0.01"
                                min={-item.currentStock / factor}
                                onChange={(e) => {
                              ...
                              <div className="w-[100px]">
                                <Select
                                  ...
                                  onValueChange={(val) => updateItem(item.id, "selectedUom", val)}
                                >
                                  <SelectTrigger className="h-9">
                                    <SelectValue placeholder="Unit" />
                                  </SelectTrigger>
                                  <SelectContent>
                                    {item.availableUoms.map((u: any) => (
                          ...
                          <TableCell className="text-center">
                            <div className="inline-flex flex-col items-center">
                              {item.adjustment !== 0 ? (
                                <>
                                  <span className={cn(
                                    ...
                                    {formatSmartUOMDisplay(newStock, item.uom)}
                                  </span>
                                  {newStock < 0 && (
                              ...
                              {(item.adjustment !== 0 || item.selectedUom !== item.uom) && (
                                <span className={cn(
                                  "text-xs font-medium",
                                  isIncrease && "text-green-600",
                                  isDecrease && "text-red-600"
                                )}>
                                  {isIncrease ? `+${item.adjustment}` : item.adjustment}{item.selectedUom}
                                  {item.selectedUom !== item.uom && ` (${isIncrease ? '+' : ''}${formatSmartUOMDisplay(adjustmentInBase, item.uom)})`}
                                </span>
                              )}
                            </div>
                          </TableCell>
            ...
            {canAdd("stock_adjustment") && <Button
              ...
              className={cn(
                items.some(item => {
                  const uomInfo = item.availableUoms.find((u: any) => u.uom === item.selectedUom);
                  const factor = uomInfo?.conversionFactor || 1;
                  return (item.currentStock + (parseFloat(item.adjustment as any) * factor)) < -0.0001;
              ...
              disabled={
                ...
                items.some(item => {
                  const uomInfo = item.availableUoms.find((u: any) => u.uom === item.selectedUom);
                  const factor = uomInfo?.conversionFactor || 1;
                  const adjustmentValue = parseFloat(item.adjustment as any) || 0;
                  return (item.currentStock + (adjustmentValue * factor)) < -0.0001;
                })
              }
            >
