...
export default function WarehouseViewListItems({
  ...
  const [activeTab, setActiveTab] = useState(searchParams.get("type") || "regular");
  ...
  useEffect(() => {
    ...
    if (activeTab !== "regular") params.set("type", activeTab);
  ...
  }, [searchTerm, categoryFilter, stockFilter, currentPage, itemsPerPage, activeTab, router]);
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      // Item Type filter
      const matchesType = item.itemType === activeTab;

      return matchesSearch && matchesCategory && matchesStock && matchesType;
    });
  }, [items, searchTerm, categoryFilter, stockFilter, activeTab]);
    ...
    <div className="space-y-4">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full max-w-[400px] grid-cols-2">
          ...
          <TabsTrigger value="processing">Processing Items</TabsTrigger>
        </TabsList>
                ...
                return (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-right">
                      ...
                      >
                        {formatSmartUOMDisplay(item.quantity, item.baseUom || item.uom)}
                      </span>
                    </TableCell>
                    <TableCell className="text-right">
      ...
      {editingItem && (
        <AddItemModal
          ...
          initialData={{
            ...
            // 🆕 Multi-UOM fields for pre-population
            baseUom: editingItem.baseUom || editingItem.uom,
            selectedUoms: editingItem.availableUoms?.map((u: any) => u.uom) || [editingItem.uom],
            uomLocked: editingItem.uomLocked || false,
          }}
        />
      )}
