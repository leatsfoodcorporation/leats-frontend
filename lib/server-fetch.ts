...
/**
 * Fetch combo products for homepage
 */
export async function fetchComboHomepageProducts(params: {
  limit?: number;
} = {}) {
  try {
    const queryParams = new URLSearchParams();
    ...
    const res = await fetch(
      `${API_URL}/api/online/frontend/combo-homepage?${queryParams.toString()}`,
      {
        cache: 'no-store', // Always get fresh data
      }
    );
    ...
    console.error(`Error fetching combo homepage products:`, error);
    return [];
  }
}
...
export async function fetchSubcategoryById(id: string) {
  try {
    const res = await fetch(`${API_URL}/api/online/frontend/subcategories/${id}`, {
      cache: 'no-store', // Always get fresh data
    });
    if (!res.ok) return null;
    const data = await res.json();
...
/**
 * Fetch enabled homepage badges (sorted by sortOrder)
 */
export async function fetchHomepageBadges() {
  try {
    const res = await fetch(`${API_URL}/api/online/badges/homepage`, {
      cache: 'no-store',
      next: { revalidate: 0 }
    });
    if (!res.ok) return [];
